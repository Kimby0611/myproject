...
const Notice = () => {
  ...
  const fetchPosts = (option = "title", query = "") => {
    let url = "http://localhost:8080/api/notice";
    if (query) {
      url = `http://localhost:8080/api/notice/search?option=${encodeURIComponent(
        option
  ...
  const ViewPost = (postNum) => {
    navigate(`/view/${postNum}`);
  };

  const editableRanks = ["사장", "이사", "부장", "차장", "과장"];
  return (
    <div className="notiwrap">
      ...
      <div className="footwrap">
        ...
        <div className="footer">
          {(location.pathname === "/notice" && editableRanks.includes(rank)) ||
