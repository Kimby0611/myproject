...
const PostPage = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await fetch(`http://localhost:8080/api/notice/${num}`);
        if (response.ok) {
  ...
  const EditButton = () => {
    navigate(`/WritePage/${num}`, { state: { post } });
  };
    ...
    if (window.confirm("정말로 이 게시글을 삭제하시겠습니까?")) {
      try {
        const response = await fetch(
          `http://localhost:8080/api/notice/${num}`,
          {
            method: "DELETE",
            headers: {
        ...
        if (response.ok) {
          alert("게시글이 삭제되었습니다.");
          navigate("/notice", { state: { refresh: true } });
        } else {
          alert("게시글 삭제에 실패했습니다.");
          navigate(`/view/${num}`, { state: { refresh: true } });
        }
      } catch (error) {
